...
factpal <- colorFactor(c( # mostly, from ColorBlindness package
  ...
  "#24FF24", # 5
  "yellow" # 6
  ), as.factor(mapb_gentrified$Cluster))	# define color palette

# leaflet map 
gent_map <- leaflet(mapb_gentrified) %>% 
  addTiles() %>% 
  addPolygons(weight = 3, opacity = 1, color = ~factpal(as.factor(mapb_gentrified$Cluster))) %>% 
  addLegend("bottomright", 
            pal = factpal, 
            values = as.factor(mapb_gentrified$Cluster),
            title = "Cluster", 
            labFormat = labelFormat())
gent_map

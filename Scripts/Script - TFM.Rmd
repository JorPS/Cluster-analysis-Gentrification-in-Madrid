...
This dataset contains information per district about sociodemographic variables by neighborhood and district in Madrid in 2020 and 2021. This data allow us to approximate the social class without the unemployment rate, using the mean household income. Also allow us to see the immigration and voter turnout rates. Also, we can calculate the growth of each of the variables from one year to the next one.
...
```{r}
df_19_scaled %>% select(-Neighborhood) %>% 
  cor() %>% 
  corrplot()
```
...
fviz_nbclust(scale(df_19_num), kmeans, method = 'wss', k.max = 20, nstart = 500)
fviz_nbclust(scale(df_19_num), kmeans, method = 'silhouette', k.max = 20, nstart = 500)
fviz_nbclust(scale(df_19_num), kmeans, method = 'gap_stat', k.max = 10, nstart = 100, nboot = 500) # as the other methods show a low ideal number of clusters that cannot describe the complex information in the dataset, I choose to use 9 clusters as it will be a large number of clusters that can explain the differences between neighborhoods across clusters, and it shows that more clusters are not going to add significant information
```
...
fit.km_19 <- eclust(df_19_num, "kmeans", stand = T, k = k, graph = T) # alternative method with plot
...
par(mar=c(15,4,1,1))
barplot(centers[1,], las=2, col="navajowhite3")
barplot(centers[2,], las=2, col="navajowhite3")
barplot(centers[3,], las=2, col="navajowhite3")
barplot(centers[4,], las=2, col="navajowhite3")
barplot(centers[5,], las=2, col="navajowhite3")
barplot(centers[6,], las=2, col="navajowhite3")
# barplot(centers[7,], las=2, col="navajowhite3")
# barplot(centers[8,], las=2, col="navajowhite3")
# barplot(centers[9,], las=2, col="navajowhite3")

# hierarchical method
hcenters=as.matrix(t(hmodel))
par(mar=c(13,8,1,1))
barplot(hcenters[1,], las=2, col="chocolate")
barplot(hcenters[2,], las=2, col="chocolate")
barplot(hcenters[3,], las=2, col="chocolate")
barplot(hcenters[4,], las=2, col="chocolate")
barplot(hcenters[5,], las=2, col="chocolate")
barplot(hcenters[6,], las=2, col="chocolate")
...
mapb_km <- mapb %>% left_join(df_neighborhood_clusters, by = "DESBDT") %>% drop_na()
...
factpal <- colorFactor(c( # mostly, from ColorBlindness package
  ...
  "#24FF24", # 5
  "yellow" # 6
  ), as.factor(mapb_km$Cluster))	# define color palette

leaflet(mapb_km) %>% addTiles() %>% addPolygons(weight=3,opacity = 1 ,color = ~factpal(as.factor(mapb_km$Cluster))) %>% addLegend("bottomright", pal = factpal, values = as.factor(mapb_km$Cluster),
                     title = "Cluster", labFormat = labelFormat())
...
leaflet(mapb_hc) %>% addTiles() %>% addPolygons(weight=3,opacity = 2 ,color = ~factpal(as.factor(mapb_hc$Cluster))) %>% addLegend("bottomright", pal = factpal, values = as.factor(mapb_hc$Cluster),
                     title = "Cluster", labFormat = labelFormat())
```
